...
!!! info
    The style guide is always a work in progress, and not all MillerIndices code
    follows the rules. When modifying MillerIndices, please fix the style violations
...
MillerIndices uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/MillerIndices.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the MillerIndices directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to MillerIndices have
