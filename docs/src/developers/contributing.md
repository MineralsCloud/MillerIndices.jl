...
```julia-repl
julia> cd(joinpath(DEPOT_PATH[1], "dev", "MillerIndices"))

julia> using Pkg

julia> Pkg.activate(".")
  Activating new project at `~/.julia/dev/MillerIndices`
...
```julia-repl
julia> cd(joinpath(DEPOT_PATH[1], "dev", "MillerIndices", "docs"))

julia> using Pkg

julia> Pkg.activate(".")
  Activating new project at `~/.julia/dev/MillerIndices/docs`
...
Then go to [our pull request page](https://github.com/MineralsCloud/MillerIndices.jl/pulls)
