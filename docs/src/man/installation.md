...
[MillerIndices](https://github.com/MineralsCloud/MillerIndices.jl).
...
version of MillerIndices, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/MillerIndices.jl"
```
...
our [GitHub repository](https://github.com/MineralsCloud/MillerIndices.jl)
for new releases.
Once we release a new version, you can update MillerIndices by typing

```@repl
using Pkg
Pkg.update("MillerIndices")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("MillerIndices")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall MillerIndices.
